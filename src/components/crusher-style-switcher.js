...
export class CrusherStyleSwitcher extends LitElement {
  static styles = css`
    ...
    .style-switcher.open {
      transform: translateX(calc(-1 * var(--crusher-spacing-6)));
    }
    ...
    .s-icon {
      ...
      right: 100%;
      border: 1px solid var(--crusher-border-primary);
      margin-right: var(--crusher-spacing-6);
      cursor: pointer;
  ...
  constructor() {
    ...
    // runtime/theme.js will initialize from stored prefs
  }
  ...
  _setActiveColor(colorVar) {
    ...
    // Safety: if a consumer kept vars on body, update there too
  ...
  render() {
    ...
    return html`
      <div class=${classMap(classes)}>
        <div class="style-switcher-toggler s-icon" @click=${this._togglePanel}>
          <i class="fas fa-cog fa-spin"></i>
        </div>
        <div class="day-night s-icon" @click=${this._toggleDarkMode}>
          <i class=${this.isDarkMode ? 'fas fa-sun' : 'fas fa-moon'}></i>
        </div>
        <h4>Theme Colors</h4>
        <div class="colors">
          ${Object.entries(this.colors).map(([name, colorVar]) => html`
        ...
        <div class="colors" style="gap: 8px;">
          ${['glass','brutal','neumorph','neobrutal'].map(t => html`
